...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
	...
	"""
	m=reconcile_countries_by_name(plot_countries,read_csv_as_nested_dict("isp_gdp.csv","Country Code",",",'"'))
	n={}
	o=set()
	p=set()
	q=set()
	with open(gdpinfo['gdpfile'],'rt') as csvfile:
		...
		for cow in reader:
			for i in plot_countries:
				country=plot_countries[i]
				if country==cow[gdpinfo['country_name']] and cow[year]!='':
					n[i]=math.log10(float(cow[year])) 
